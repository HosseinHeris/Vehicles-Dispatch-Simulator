...
class TemplateSimulatorSession:
    def __init__(
        ...
        env_name: str = "TaxiDispatcher",
    ):
        # Initialize python api for simulator
        self.simulator = Simulation(
            ClusterMode=setting.ClusterMode,
            DemandPredictionMode=setting.DemandPredictionMode,
            DispatchMode=setting.DispatchMode,
            VehiclesNumber=setting.VehiclesNumber,
            TimePeriods=setting.TIMESTEP,
            LocalRegionBound=setting.LocalRegionBound,
            SideLengthMeter=setting.SideLengthMeter,
            VehiclesServiceMeter=setting.VehiclesServiceMeter,
            NeighborCanServer=setting.NeighborCanServer,
            FocusOnLocalRegion=setting.FocusOnLocalRegion,
        )
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        self.current_obs = {}
        self.current_obs = self.simulator.GetNextStateFunction()
        self.current_obs['_reward'] = self.simulator.RewardFunction()
        self.current_obs['_terminal'] = self.simulator.GetTerminal()
        # print([type(i) for i in self.current_obs.values()])
    ...
    def episode_start(self, config: Dict[str, Any]):
        ...
        self.simulator.UpdateSimParamters(self.sim_config)
        self.simulator.EpisodeReset()

    def episode_step(self, action: Dict[str, Any]):
        """ Called for each step of the episode """
        # Add simulator step api here using action from Bonsai platform

        self.simulator.EpisodeStep(dispatch_action=action)
